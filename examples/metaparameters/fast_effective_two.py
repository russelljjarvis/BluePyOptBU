        ...
        if 'Spikecount_3.0x'==self.test.name or 'Spikecount_1.5x'==self.test.name:
            ...
            if feature_name in check_list:
                ...
                if score>1.0:
                    ...
                    #141525000.0 hit AP2_AP1_begin_width_diff_1.5x
                    #2.15895 hit AHP_depth_diff_1.5x    
...
for tt in nu_tests:
    ...
    if 'Spikecount_1.5x'in tt.name:
        spike_obs.append(tt.observation)
#spike_obs = sorted(spike_obs,reverse=True,key='mean')
spike_obs = sorted(spike_obs, key=lambda k: k['mean'],reverse=True)
#check_list.extend(list(aug_nu_tests.keys()))
...
for cnt,tt in enumerate(nu_tests):
    ...
    if feature_name in check_list:
        if 'Spikecount_3.0x' in tt.name or 'Spikecount_1.5x'in tt.name:# or 'spike_width2_3.0x' in tt.name or 'AHP_time_from_peak_3.0x' in tt.name:# or 'isi' in tt.name or 'cv' in tt.name:
            #ft = NUFeatureAllen(tt,model,check_list,cnt,target,spike_obs=spike_obs)
...
# +
#assert 1==2
#from bluepyopt.deapext.algorithms import eaAlphaMuPlusLambdaCheckpoint

'''
final_pop, hall_of_fame, logs, hist = optimisation.run(max_ngen=150, cp_filename='results_100.p',continue_cp=True)

optimisation.eaAlphaMuPlusLambdaCheckpoint
